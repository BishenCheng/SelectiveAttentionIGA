...
import { WebGazerContext } from './index'; // 导入上下文
...
const EyeTracker = () => {
  ...
  useEffect(() => {
    // 在 WebGazer.js 初始化前设置indexDB
    window.saveDataAcrossSessions = true;
    const initializeWebGazer = async () => {
      const webgazer = await WebGazerContext;
      await webgazer
        .setRegression("ridge")
        .setGazeListener((data, clock) => {
          gazeListeners.forEach(listener => listener(data, clock));
        })
      ...
      webgazer
        ...
        .applyKalmanFilter(true); //打开卡尔曼滤波
  ...
  // 修改后的导出数据的函数
  const exportIndexDBData = async () => {
    try {
      // 重新初始化 localforage 以确保使用正确的配置
      ...
      // 获取指定密钥的数据
      const data = await configuredLocalforage.getItem('webgazerGlobalData');
      const dataStr = JSON.stringify(data, null, 2);
      const blob = new Blob([dataStr], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'webgazer_global_data.json';
      a.click();
...
export { EyeTracker,addGazeListener };
